...
Env vars and secrets are defined once under `personal/devenv/env/` and regenerated by `env/setup.sh` (run by the host user). Flow: **1Password → macOS keychain → per-scope `*.mise.toml`**.

- `env/secrets` — manifest mapping a keychain service name → `op://` reference → target keychain (`host` / `sandbox` / `both`) → 1Password account (so secrets can be pulled from multiple accounts via `op read --account`).
- `env/{ai-sandbox,work,personal,main-user}.mise.toml` — templates copied to `~ai-sandbox/.config/mise/config.toml`, `/Users/Shared/dev/work/.mise.toml`, `/Users/Shared/dev/personal/.mise.toml`, and `~/.config/mise/conf.d/devenv.toml` (main-user-global). Secrets are *never* stored literally — each is a `{{ exec(command='security find-generic-password …') }}` lookup resolved at mise load time (with a `|| true` fallback so a missing entry yields `""` instead of breaking mise).
- `env/setup.sh` reads each secret from 1Password via `op` and writes it into the host and/or sandbox login keychain. The sandbox can't reach `op` or the host keychain, so the host runs the sync and populates the sandbox keychain via `sudo -u ai-sandbox security …`.
...
Work vs personal env vars are **directory-scoped** — `work/.mise.toml` loads only under `work/`, `personal/.mise.toml` only under `personal/`. Vars that must be available everywhere for the main user (regardless of cwd, e.g. at the `/Users/Shared/dev` root) go in `main-user.mise.toml` instead, which mise loads globally from `~/.config/mise/conf.d/`.

# Environment

`mise` is installed and available. Tools provisioned via the ai-sandbox mise config (`env/ai-sandbox.mise.toml`):
